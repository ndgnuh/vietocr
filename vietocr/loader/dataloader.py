...
def letterbox(image, height, width):
    ...
    target = Image.new(size=(width, height), mode="RGB")
    x = abs(width - image.size[0]) // 2
    y = abs(height - image.size[1]) // 2
...
class OCRDataset(Dataset):
    ...
    def read_data(self, idx):
        ...
        if self.letterbox:
            img_bw = letterbox(img, self.image_height, self.image_max_width)
            img_bw = TF.to_tensor(img_bw)
        else:
...
def collate_dlen_labels(batch):
    ...
    rs = {
        ...
        'tgt_output': torch.tensor(tgt_input.T, dtype=torch.long),
    }
    return rs
